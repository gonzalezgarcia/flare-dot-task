  ...
  plugin.trial = function(display_element, trial) {
    ...
    var after_response = function(info) {
        ...
        if (responses_key.length <= trial.max_Resp) {
          response = info;
          if(response==null){                             // 1st response: we just store it
          ...
          else{                                           // From 2nd response onward, we check if the key press deteced are repetitions by mistake 
               if (responses_key.includes(response.key)){ // Otherwise, the last(s) response(s) won't be store - 
                 // DO NOTHING //                         // KeyboardListener is cancelled when noResposnes >= maxResponses 
               } else {
                responses_rt.push(response.rt)
                responses_key.push(response.key)
                responses_char = responses_char+jsPsych.pluginAPI.convertKeyCodeToKeyCharacter(response.key)
